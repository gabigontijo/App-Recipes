...
describe('Favorite recipes screen', () => {
  ...
  describe('50 - Implemente os elementos da tela de receitas favoritas (cumulativo com os atributos em comum com a tela de receitas feitas), respeitando os atributos descritos no protótipo', () => {
    it('Todos os data-testids, cumulativo com os atributos em comum com a tela de receitas feitas, estão disponíveis', () => {
      ...
      cy.get('[data-testid="filter-by-meal-btn"]');
  ...
  describe('51 - Desenvolva a tela de modo que, caso a receita do card seja uma comida, ela deve possuir: a foto da receita,  nome, categoria, nacionalidade, um botão de compartilhar e um de \"desfavoritar\"', () => {
  ...
  describe('52 - Desenvolva a tela de modo que, caso a receita do card seja uma bebida, ela deve possuir: a foto da receita,  nome, se é alcoólica ou não, um botão de compartilhar e um de \"desfavoritar\"', () => {
    ...
    it('Verifica a cobertura de 45% da tela de Receitas Favoritas', () => {
      cy.task('getCoverage', getId()).its('FavoriteRecipes.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
      cy.task('getCoverage', getId()).its('FavoriteRecipes.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
      cy.task('getCoverage', getId()).its('FavoriteRecipes.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
    });
  });

  describe('53 - Desenvolva a solução de modo que o botão de compartilhar deve copiar a URL da tela de detalhes da receita para o clipboard', () => {
  ...
  describe('54 - Desenvolva a solução de modo que o botão de \"desfavoritar\" deve remover a receita da lista de receitas favoritas do `localStorage` e da tela', () => {
  ...
  describe('55 - Implemente 2 botões que filtram as receitas por comida ou bebida e um terceiro que remove todos os filtros', () => {
    it('Ao clicar no botão "Meal" as receitas devem ser filtradas por comidas', () => {
      cy.get('[data-testid="filter-by-meal-btn"]').click();
  ...
  describe('56 - Redirecione a pessoa usuária ao clicar na foto ou no nome da receita, a rota deve mudar para a tela de detalhes daquela receita', () => {
    ...
    it('Verifica a cobertura de 90% da tela de Receitas Favoritas', () => {
      cy.task('getCoverage', getId()).its('FavoriteRecipes.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
      cy.task('getCoverage', getId()).its('FavoriteRecipes.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
      cy.task('getCoverage', getId()).its('FavoriteRecipes.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
    });
  });
});
