...
describe('37 - Desenvolva a tela de modo que contenha uma imagem da receita, o título, a categoria em caso de comidas e se é ou não alcoólico em caso de bebidas, uma lista de ingredientes com suas respectivas quantidades e instruções', () => {
  it('verifica elementos de uma receita de comida', () => {
    cy.visit('http://localhost:3000/meals/52771/in-progress', {
      onBeforeLoad(win) {
        win.fetch = fetchMock;
      },
    });
...
describe('39 - Implemente uma lógica que ao clicar no checkbox de um ingrediente, o nome dele deve ser \"riscado\" da lista', () => {
  ...
  it('Verifica a cobertura de 45% da Tela de Progresso da Receita', () => {
    cy.task('getCoverage', getId()).its('RecipeInProgress.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('RecipeInProgress.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('RecipeInProgress.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
  });
});
...
describe('40 - Salve o estado do progresso, que deve ser mantido caso a pessoa atualize a página ou volte para a mesma receita', () => {
  it('salva o progresso de uma receita de comida em andamento', () => {
    ...
    cy.get('[data-testid="1-ingredient-step"]')
      .find('input[type="checkbox"]')
...
describe('41 - Desenvolva a lógica de favoritar e compartilhar. A lógica da tela de detalhes de uma receita se aplica aqui', () => {
  ...
  it('favorita comida', () => {
    ...
    cy.window().then((win) => {
      const favoriteRecipes = JSON.parse(win.localStorage.getItem('favoriteRecipes'));
...
describe('43 - Redirecione a pessoa usuária após clicar no botão de finalizar receita (\"Finish Recipe\"), para a página de receitas feitas, cuja rota deve ser `/done-recipes`', () => {
  it('adiciona a comida feita no localStorage', () => {
    cy.visit('http://localhost:3000/meals/52771/in-progress', {
      onBeforeLoad(win) {
        win.fetch = fetchMock;
      },
    ...
    cy.window().then((win) => {
      const doneRecipes = JSON.parse(win.localStorage.getItem('doneRecipes'));
      ...
      expect(doneRecipes[0]).to.deep.eq(expectedDoneRecipes);
    });
  });

  it('adiciona a bebida feita no localStorage', () => {
    cy.visit('http://localhost:3000/drinks/178319/in-progress', {
      onBeforeLoad(win) {
        win.fetch = fetchMock;
      },
  ...
  it('Verifica a cobertura de 90% da Tela de Progresso da Receita', () => {
    cy.task('getCoverage', getId()).its('RecipeInProgress.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('RecipeInProgress.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('RecipeInProgress.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
});
